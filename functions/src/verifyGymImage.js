  ...
  try {
    ...
    if (GEMINI_API_KEY) {
      try {
        ...
        return { success: true, verified, modelUsed };
      } catch (err) {
    ...
    throw new HttpsError('internal', `Failed to analyze the image. All models in the cascade failed: ${JSON.stringify(errors)}`);

  } catch (error) {
    console.error('[verifyGymImage] Error:', error.message);
    if (error instanceof HttpsError) throw error;
    throw new HttpsError('internal', error.message || 'Failed to analyze the image. Please make sure the photo is clear.');
  }
});
